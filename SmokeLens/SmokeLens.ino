...
// =========================
const char *INFERENCE_MODEL_VERSION = "smokelens_mlp_7x2x4_seed46_pm_guard_v1";
const int MODEL_NUM_FEATURES = 7;
const int MODEL_NUM_HIDDEN = 2;
const int MODEL_NUM_CLASSES = 4;
const int MODEL_CIGARETTE_SMOKE_INDEX = 3;
const uint16_t PM1_SMOKE_THRESHOLD = 10;
const uint16_t PM25_SMOKE_THRESHOLD = 15;
const uint16_t PM10_SMOKE_THRESHOLD = 15;
const uint8_t PM_REQUIRED_HITS = 2;
...
LocalInferenceResult runLocalInference(float voc_mv, float co_mv, float pm1_0,
  ...
  float smokeProbability = exps[MODEL_CIGARETTE_SMOKE_INDEX] / sumExp;
  uint8_t pmHits = 0;
  pmHits += pm1_0 >= PM1_SMOKE_THRESHOLD;
  pmHits += pm2_5 >= PM25_SMOKE_THRESHOLD;
  pmHits += pm10 >= PM10_SMOKE_THRESHOLD;
  bool pmSmokeDetected = pmHits >= PM_REQUIRED_HITS;
  bool cigaretteDetected = smokeProbability >= 0.5f || pmSmokeDetected;
  float smokeScore = smokeProbability;
  if (pmSmokeDetected) {
    smokeScore = max(smokeScore, static_cast<float>(pmHits) / 3.0f);
  }
  
  LocalInferenceResult res;
  res.className = cigaretteDetected ? "cigarette_smoke" : "normal_air";
  res.score = cigaretteDetected ? smokeScore : 1.0f - smokeProbability;
  res.cigaretteDetected = cigaretteDetected;
  return res;
}
